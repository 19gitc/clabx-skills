...
## PPT / PPTX output decision

The HTML deck is the native artifact of this skill, but many users say "PPT"
when they mean an editable PowerPoint file. Make the output target explicit
before implementation:

- **If the requested deliverable is `.pptx`, default to editable PPTX.** Use
  native PowerPoint objects via `pptxgenjs` or an equivalent local library:
  text boxes for all text, editable shapes for cards / arrows / diagrams,
  tables for tabular content, and chart primitives where practical. Keep
  slide text selectable and editable in PowerPoint/WPS.
- **Do not satisfy a PPTX request by screenshotting HTML slides unless the
  user explicitly wants a non-editable visual export or accepts the tradeoff.**
  A PPTX made from full-slide PNGs is a preview / fidelity fallback, not an
  editable deck.
- **If HTML presenter mode is also valuable, produce it as a companion
  artifact.** The editable PPTX remains the primary classroom/business
  deliverable; the HTML version can be used for browser presentation and visual
  verification.
- **When using screenshot export, label it clearly.** In the final summary,
  say "image-based PPTX, not editable" and provide the editable source artifact
  if one exists.
- `baoyu-slide-deck` can be consulted for generation discipline: analyze
  content, choose style/audience/slide count, keep source/outline artifacts,
  back up existing outputs, and merge reproducibly. If its current
  implementation or project samples use native PowerPoint shapes/text, treat
  those as editable PPTX construction references. Always inspect the script or
  resulting PPTX structure before assuming editability.
